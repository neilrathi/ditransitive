...
export function Instructions() {
  ...
  const li2 = round.get("instructions") == "train" ? `Memorize the label for each image to the best of your ability.` :
    ...
    (round.get("instructions") == "choice" || round.get("instructions") == "example") & player.get("role") == 'director' ? <span>You need to communicate to the other participant which image is the target using <b>full sentences and the target verb.</b></span> :
    (round.get("instructions") == "choice" || round.get("instructions") == "example") & player.get("role") == 'guesser' ? <span>You need to communicate with the other participant to figure out which image is the target. Once you have determined the target, click on it.</span> : null;
  const li3 = round.get("instructions") == "train" ? `Your partner will see and memorize the same labels.` :
    ...
    round.get("instructions") == "example" & player.get("role") == 'guesser' ? `You will first be shown two stimuli with possible example descriptions that the director might give you.` :
  ...
  const li4 = round.get("instructions") == "train" ? `Once you have memorized the label for one image, click continue to move to the next.` :
    ...
    round.get("instructions") == "choice" & player.get("role") == 'director' ? `You should first describe the image in one sentence to your partner.` :
    round.get("instructions") == "choice" & player.get("role") == 'guesser' ? `Wait for your partner to first describe the image before you speak to them.` : null;
